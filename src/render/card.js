function Card(item) {
  ...
  const ProgressElSec = createEl('div');
  const cProgressSec = createEl('div');
  ...
  rmBtn.classList.add('btn-floating', 'waves-effect', 'waves-light', 'right', 'red', 'darken-1', 'z-depth-2', 'remove');
  titleEl.classList.add('card-title', 'activator', 'grey-text', 'text-darken-4');
  ...
  cProgressSec.classList.add('progress', 'sec');
  ProgressElSec.classList.add('indeterminate');

  cProgressSec.appendChild(ProgressElSec);
  cProgress.appendChild(progressEl);
  infoEl.appendChild(infoTxt);
  infoEl.appendChild(cProgress);
  infoEl.appendChild(cProgressSec);
  ...
  optionsEl.appendChild(bottomEl);
...
Card.prototype.disableExtendedMode = function(enable = false) {
  if(this.cSel ==! undefined) {
    if(this.extendedMode || enable) {
